                    ...
                    <div class="d-flex gap-2">
                        @can('view products')
                            <form method="GET" action="{{ route('products.index') }}" class="d-flex align-items-center">
                                <select name="warehouse_id" id="filter_warehouse" class="form-select form-select-sm"
                                ...
                                <tbody>
                                    @forelse($products as $product)
                                        <tr>
                                                ...
                                                <div class="warehouse-badges">
                                                    @foreach ($product->warehouses as $warehouse)
                                                        <span class="badge bg-info warehouse-badge"
                                                            title="{{ $warehouse->pivot->quantity }} copë">
                                                            {{ $warehouse->name }}
                                                            @if ($warehouse->pivot->quantity > 0)
                                                                <span
                                                                    class="badge bg-light text-dark ms-1">{{ $warehouse->pivot->quantity }}</span>
                                                            @endif
                                                        </span>
                                                    @endforeach
                                                </div>
                                            ...
                                            <td>
                                                <strong>{{ number_format($product->unit_price, 2) }}
                                                    {{ $product->currency ? $product->currency->symbol : '' }}</strong>
                                            </td>
                                                    ...
                                                    <div class="hstack gap-1">
                                                        @can('view products')
                                                            <button type="button" class="btn btn-sm btn-info btn-show"
                                                                data-id="{{ $product->id }}" title="Shiko">
                                                                <i class="ri-eye-line"></i>
                                                            </button>
                                                        @endcan

                                                        @can('edit products')
                                                            <button type="button" class="btn btn-sm btn-warning btn-history"
                                                                data-id="{{ $product->id }}" title="Histori">
                                                                <i class="ri-history-line"></i>
                                                            </button>

                                                            <button type="button" class="btn btn-sm btn-primary btn-edit"
                                                                data-id="{{ $product->id }}" title="Modifiko">
                                                        ...
                                                        @can('delete products')
                                                            <button type="button" class="btn btn-sm btn-danger btn-delete"
                                                                data-id="{{ $product->id }}" title="Fshij">
            ...
            `;
                    ...
                    fetch('/products/' + id, {
                        ...
                        `;
                            ...
                            const selectedWarehouseIds = data.warehouses ? data.warehouses.map(w => w
                                .id) : [];
                    ...
                    `;
                            ...
                            $(document).off('change', '#edit_category_id').on('change',
                                '#edit_category_id',
                                function() {
                                    const selectedCategory = categories.find(c => c.id == $(this)
                                        .val());
                                    if (selectedCategory) {
                                    ...
                                    `;
                                            ...
                                            $('#edit_storage, #edit_ram, #edit_color').prop(
                                                'required', true);
                                        } else {
