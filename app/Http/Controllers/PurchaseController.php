...
class PurchaseController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
                ...
                if (!empty($item['imei_numbers'])) {
                    ...
                    if ($imeiCount != $quantity) {
                        ...
                        if ($request->ajax()) {
                            ...
                            return redirect()->back()
                                ->with('error', "Produkti #" . ($itemIndex + 1) . ": Numri i IMEI duhet të jetë i barabartë me sasinë ({$quantity}). Ju keni vendosur {$imeiCount} IMEI.")
                                ->withInput();
                        }
                    }
                    ...
                    if (count($uniqueImei) != count($imeiArray)) {
                        ...
                        if ($request->ajax()) {
                            ...
                            return redirect()->back()
                                ->with('error', "Produkti #" . ($itemIndex + 1) . ": Ka IMEI të dubluar. Çdo IMEI duhet të jetë unik.")
                                ->withInput();
                        }
                    }
                    ...
                    foreach ($imeiArray as $imei) {
                        if (!preg_match('/^\d{15}$/', $imei)) {
                            ...
                            if ($request->ajax()) {
                                ...
                                return redirect()->back()
                                    ->with('error', "Produkti #" . ($itemIndex + 1) . ": IMEI '{$imei}' nuk është valid. IMEI duhet të jetë 15 shifra.")
                                    ->withInput();
                            }
                        }
                    }
                    ...
                    foreach ($imeiArray as $imei) {
                        ...
                        if ($existingImei) {
                            ...
                            if ($request->ajax()) {
                                ...
                                return redirect()->back()
                                    ->with('error', "IMEI {$imei} ekziston tashmë në sistem (Purchase #{$existingImei->purchase->purchase_number}).")
                                    ->withInput();
                            }
                        }
                        ...
                        if (in_array($imei, $allImeiNumbers)) {
                            ...
                            if ($request->ajax()) {
                                ...
                                return redirect()->back()
                                    ->with('error', "IMEI {$imei} është përdorur më shumë se një herë në këtë blerje.")
                                    ->withInput();
                            }
                        }
            ...
            DB::commit();
            if ($request->ajax()) {
                return response()->json(['success' => true, 'message' => 'Purchase created successfully', 'url' => route('purchases.index')], 200);
            } else {
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            if ($purchase->order_status === 'Received') {
                foreach ($purchase->items as $itemIndex => $item) {
                    $product = Product::find($item->product_id);
                    if ($product) {
                    ...
                    if (!empty($item['imei_numbers'])) {
                        ...
                        if (count($uniqueImei) != count($imeiArray)) {
                            DB::rollBack();
                            return redirect()->back()
                                ->with('error', "Produkti #" . ($itemIndex + 1) . ": Ka IMEI të dubluar.")
                                ->withInput();
                        }
                        ...
                        foreach ($imeiArray as $imei) {
                            if (!preg_match('/^\d{15}$/', $imei)) {
                                DB::rollBack();
                                return redirect()->back()
                                    ->with('error', "Produkti #" . ($itemIndex + 1) . ": IMEI '{$imei}' nuk është valid.")
                                    ->withInput();
                            }
                        }
                        ...
                        if (!empty($conflictingImeis)) {
                            DB::rollBack();
                            return redirect()->back()
                                ->with('error', 'IMEI ' . reset($conflictingImeis) . ' ekziston tashmë në sistem.')
                                ->withInput();
                        }
